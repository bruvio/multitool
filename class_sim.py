...
printf,lun4,format='(A)',' psi_omp dsrad_omp dsrad_face_omp  ds_omp r_omp z_omp hrho_omp sh_omp bfi_omp jpar_omp jpari_omp jtarg_omp jtargi_omp gam_omp gamro_omp gamroe_omp pflxd_omp denel_omp te_omp ti_omp vi_omp mach_omp da_omp dm_omp dha_omp soun_omp sirec_omp pre_omp preel_omp prestat_omp pretot_omp qeflxd_omp qiflxd_omp qepcd_omp qepcdd_omp qepcv_omp qipcd_omp qipcdd_omp qipcv_omp qpare_omp qpari_omp qpartot_omp denpe_omp denpi_omp totpden_omp dperp_omp chii_omp chie_omp'
    ...
    if tran is None:
        ...
        if tran<10:
            fullpath = self.fullpath +str(0)+ str(tran)
        else:
            fullpath = self.fullpath+str(tran)

    ade = ep.row(fullpath,'DENEL', Region)
    adi = ep.row(fullpath,'DEN', Region)
    asoun = ep.row(fullpath,'SOUN', Region)
    asirec = ep.row(fullpath,'SIREC', Region)
    psi = ep.row(fullpath,'PSI', Region)
    ate = ep.row(fullpath,'TEVE', Region)
    ati = ep.row(fullpath,'TEV', Region)
    apot = ep.row(fullpath,'POT', Region)
    rsepx = ep.row(fullpath,'RSEPX', Region)
    pre = ep.row(fullpath,'PRE', Region)
    preel = ep.row(fullpath,'PREEL', Region)
    pretot = ep.row(fullpath,'PREHYD', Region)
    prestat = ep.row(fullpath,'PRETOT', Region)
    vpi = ep.row(fullpath,'VPI', Region)
    vpe = ep.row(fullpath,'VPE', Region)
    mach = ep.row(fullpath,'PMACH', Region)
    pflxd = ep.row(fullpath,'PFLXD', Region)
    agamroe = ep.row(fullpath,'GAMROE', Region)
    jpar = ep.row(fullpath,'JPAR', Region)
    jpari = ep.row(fullpath,'JPARI', Region)
    jtarg = ep.row(fullpath,'JTARG', Region)
    jtargi = ep.row(fullpath,'JTARGI', Region)
    agam = ep.row(fullpath,'GAM', Region)
    agamro = ep.row(fullpath,'GAMRO', Region)
    ada = ep.row(fullpath,'DA', Region)
    adm = ep.row(fullpath,'DM', Region)
    adha = ep.row(fullpath,'DHA', Region)
    qeflxd = ep.row(fullpath,'QEFLXD', Region)
    qepcd = ep.row(fullpath,'QEPCD', Region)
    qepcdd = ep.row(fullpath,'QEPCDD', Region)
    qepcv = ep.row(fullpath,'QEPCV', Region)
    qiflxd = ep.row(fullpath,'QIFLXD', Region)
    qipcd = ep.row(fullpath,'QIPCD', Region)
    qipcdd = ep.row(fullpath,'QIPCDD', Region)
    qipcv = ep.row(fullpath,'QIPCV', Region)
    bfi = ep.row(fullpath,'BFI', Region)
    sh = ep.row(fullpath,'SH', Region)
    rmesh = ep.row(fullpath,'RMESH', Region)
    zmesh = ep.row(fullpath,'ZMESH', Region)
    hrho = ep.row(fullpath,'HRHO', Region)
    dperp = ep.row(fullpath,'DPERP', Region)
    chii = ep.row(fullpath,'CHII', Region)
    chie = ep.row(fullpath,'CHIE', Region)
    denpe = ep.row(fullpath,'DENPE', Region)
    denpi = ep.row(fullpath,'DENPI', Region)
    totpden = ep.row(fullpath,'TOTPDEN', Region)
    qpare = ep.row(fullpath,'QPARE', Region)
    qpari = ep.row(fullpath,'QPARI', Region)
    qpartot = ep.row(fullpath,'QPARTOT', Region)

    # arcx=ep.row(fullpath,'RCX',Region)
    # asqicx=ep.row(fullpath,'SQICX',Region)
    # aripg=ep.row(fullpath,'RIPG',Region)
    # ava0r=ep.row(fullpath,'VA0R',Region)
    # aeneuta=ep.row(fullpath,'ENEUTA',Region)

    # aqeflxl=ep.row(fullpath,'QEFLXL',Region)
    # aqiflxl=ep.row(fullpath,'QIFLXL',Region)
    # aqeflxd=ep.row(fullpath,'QEFLXD',Region)
    # aqiflxd=ep.row(fullpath,'QIFLXD',Region)
    # apflxd=ep.row(fullpath,'PFLXD',Region)
    #
    # aqirocdl=ep.row(fullpath,'QIROCDL',Region)
    # aqirocvl=ep.row(fullpath,'QIROCVL',Region)
    # aqerocdl=ep.row(fullpath,'QEROCDL',Region)
    # aqerocvl=ep.row(fullpath,'QEROCVL',Region)
    # else:

...
class Plot():
        ...
        if plot_dict:
            ...
            for key, val in plot_dict.items():
                ...
                if key == 'prof_impemiss_defs':
                    self.plot_impemiss_profiles(lineweight=3.0, alpha=0.250, legend=False)
                ...
                if key == 'imp2_2d_prad':
                    ...
                    Rrng = val['Rrng']
                    Zrng = val['Zrng']
                    self.plot_2d_imp2_prad(diagLOS, Rrng=Rrng, Zrng=Zrng, savefig=savefig)
                if key == 'nii_adas_afg':
    ...
    def plot_param_profiles(self, lineweight=2.0, alpha=0.250, legend=True):
        ...
        if not isinstance(axs, np.ndarray):
            axs.plot(x, ne, c=color, lw=lineweight, zorder=zorder)
        else:
            axs[0].plot(x, ne, c=color, lw=lineweight, zorder=zorder)
        ...
        # Te
            ...
            axs[1].plot(x, Te_hi, c=color, lw=lineweight, zorder=zorder)
            axs[1].plot(x, Te_lo, c=color, lw=lineweight, zorder=zorder)
        ...
        if isinstance(axs, np.ndarray) and len(axs) > 2:
            ...
            axs[2].semilogy(x, Srec_adf11, '--', c=color, lw=lineweight, zorder=zorder)
            axs[2].semilogy(x, Sion_adf11, c=color, lw=lineweight, zorder=zorder)
        ...
        if self.plot_dict['prof_param_defs']['include_pars_at_max_ne_along_LOS']:
            ...
            if len(axs) > 0:
                axs[0].plot(x, ne_max, '-', c='darkgray', lw=2.0, zorder=1)
            if len(axs) > 1:
                axs[1].plot(x, te_max, '-', c='darkgray', lw=2.0, zorder=1)
            if len(axs) > 3:
                axs[3].plot(x, n0_max, '-', c='darkgray', lw=2.0, zorder=1)
        ...
        if self.plot_dict['prof_param_defs']['include_sum_Sion_Srec']:
            ...
            if len(axs) > 2:
                axs[2].semilogy(x, Sion, '-', c='darkgray', lw=2.0, zorder=1)
                # axs[2].semilogy(x, Sion, '-', c='k', lw=2.0, zorder=1)
                axs[2].semilogy(x, -1.0*Srec, '--', c='darkgray', lw=2.0, zorder=1)
        ...
        if isinstance(axs, np.ndarray):
            ...
            axs[len(axs)-1].set_xlabel('Major radius on tile 5 (m)')
            if len(axs) > 0:
            ...
            if len(axs) > 3:
                axs[3].set_ylabel(r'$\mathrm{n_{H}\Delta L\/(m^{-2})}$')
        else:
            ...
            axs.set_xlabel('Major radius on tile 5 (m)')
            axs.set_ylabel(r'$\mathrm{n_{e}\/(m^{-3})}$')
    ...
    def plot_Hemiss_profiles(self, lineweight=2.0, alpha=0.250, legend=True):
        ...
        for i, line in enumerate(lines.keys()):
            ...
            axs[i].plot(x, excit+recom, '-', lw=lineweight, c=color, zorder=zorder, label=label)
            if excrec:
    ...
    def plot_impemiss_profiles(self, lineweight=2.0, alpha=0.250, legend=True):
        ...
        for i in range(len(axs)):
            ...
            if xlim:
                ...
                axs[i].set_xlim(x[0], x[-1])
            label = '$\mathrm{ph\/s^{-1}\/m^{-2}\/sr^{-1}\/nm^{-1}}$'
            axs[i].set_ylabel(label)
            if i == len(axs)-1:
                axs[i].set_xlabel('Major radius on tile 5 (m)')
                # axs[i].set_xlabel(coord)
    ...
    def plot_params_along_LOS(self, lineweight=2.0, alpha=0.250, legend=True):
        ...
        if isinstance(axs, np.ndarray) and len(axs) > 0:
            ...
            if imp2_den.any():
                ...
                axs[0].semilogy(l, imp2_den[:,1], ':', c=color, lw=lineweight, zorder=zorder, label=r'$\mathrm{n_{N1+}}$')
                ...
                if dummy==0:
                    axs[0].legend(loc='lower right', prop={'size': 18}, frameon=True, labelspacing=0.05)
        ...
        if isinstance(axs, np.ndarray) and len(axs) > 1:
            ...
            if imp2_den.any():
                axs[1].plot(l, np.sum(imp2_den, axis=1)/ne, '-', c=color, lw=lineweight, zorder=zorder, label=r'$\mathrm{n_{N}/n_{e}}$')
                if dummy==0:
                    ...
                    for i, ion_stage in enumerate(lines[at_num].keys()):
                        for line in lines[at_num][ion_stage]:
                            if line == '3996.13':
                                ...
                                axs[2].plot(l, np.asarray(excit) + np.asarray(recom), '-', lw=lineweight, c=color, zorder=zorder, label=label)
            if dummy==0:
                axs[2].legend(loc='upper right', prop={'size': 18}, frameon=True, labelspacing=0.05)
    ...
    def plot_2d_imp2_prad(self, diagLOS, Rrng=None, Zrng=None,
                          min_clip=0.0, max_clip = 1.0, savefig=False):
        ...
        for cell in self.__data2d.cells:
            cell_patches.append(patches.Polygon(cell.poly.exterior.coords, closed=True, zorder=1))
            prad.append(np.sum(cell.imp2_radpwr))
    ...
    def plot_2d_ff_fb(self, diagLOS, Rrng=None, Zrng=None, min_clip=0.0, max_clip = 1.0,
                      savefig=False):
...
if __name__=='__main__':
    ...
    fig6, ax6 = plt.subplots(nrows=1, ncols=1, figsize=(12,10), sharex=True, sharey=True)
    plt.subplots_adjust(left=left, bottom=bottom, right=right, top=top, wspace=wspace, hspace=hspace)
    ...
    plot_dict = {
        ...
        # '2d_defs': {'lines': spec_line_dict, 'diagLOS': [], 'Rrng': [2.36, 2.96], 'Zrng': [-1.73, -1.29], 'save': True},
        # 'imp2_2d_prad': {'diagLOS': [], 'Rrng': [2.36, 2.96], 'Zrng': [-1.73, -1.29], 'save': True}
    }
